...
void placeIntoBuckets(int threadId) {
	...
	int endingIndex = listSize / numThreads * (threadId + 1) - 1; 
	int bucketCapacity = listSize / numThreads;
	BucketsCollection *widdleBuckets = new BucketsCollection(bucketCapacity, numBuckets);
	//honestly Jen Baxter told me about putting an if statement here. I had exams last week so I never bothered getting to the 4 million
	//test. It ran on the browser shell compiler perfectly, so I didn't think to debug for VS.
	for (int i = startingIndex; i <= endingIndex; i++) {
		if (numBuckets == 1)
		{
			int bucket = list[i] / ((ULONGMAX / numBuckets));
			widdleBuckets->addItem(bucket, list[i]);
		}
		else
		{
	...
	globalBuckets->copyOneBucketsIntoAnotherBuckets(*widdleBuckets);
...
void sortEachBucket(int threadId) {
	unsigned long *items = globalBuckets->getBucketArray(threadId);
	int size = globalBuckets->getNumItemsInABucket(threadId);
...
void combineBuckets(int threadId) {
	...
	globalBuckets->copyBucketToAnotherArray(threadId, list, size);
